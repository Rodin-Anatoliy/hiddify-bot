...
func (m *mockHiddify) ListPanelUsers(_ context.Context) ([]subscription.PanelUser, error) {
	...
	for tgID, uuid := range m.byTelegram {
		id := tgID
		out = append(out, subscription.PanelUser{UUID: uuid, TelegramID: &id})
	}
...
func (m *mockHiddify) CreateUser(_ context.Context, req subscription.CreateUserRequest) (*subscription.CreatedUser, error) {
	if req.Name == "" {
...
func TestRegisterOrGet_NewUser_AutoLinked(t *testing.T) {
	...
	uc := service.NewUserUseCase(repo, newMockHiddify(map[int64]string{42: "uuid-abc"}), logger.New("debug"))
...
func TestLinkManually_ExistingUser(t *testing.T) {
	...
	uc := service.NewUserUseCase(repo, newMockHiddify(map[int64]string{1: "target-uuid"}), logger.New("debug"))
...
func TestListUnboundPanelUsers(t *testing.T) {
	...
	h.unbound = []subscription.PanelUser{
...
func TestListPanelUserViews_EnrichesWithLocalBotState(t *testing.T) {
	...
	h.unbound = []subscription.PanelUser{{UUID: "uuid-no-tg", Name: "No TG"}}
	uc := service.NewUserUseCase(repo, h, logger.New("debug"))
