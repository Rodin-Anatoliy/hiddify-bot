...
func (bot *Bot) handleUserText(c tele.Context) error {
	...
	if _, err := bot.supportUC.HandleUserMessage(ctx, service.IncomingMessage{
...
func (bot *Bot) handleReplyCallback(c tele.Context) error {
	...
	if err := bot.sessionRepo.Save(ctx, admin.Session{
	...
	if _, err := bot.b.EditReplyMarkup(c.Message(), markup.ActiveReply(targetID)); err != nil {
...
func (bot *Bot) handleAdminReply(c tele.Context) error {
	...
	if err := bot.SendText(ctx, session.TargetTgID, "📬 *Ответ поддержки:*\n\n"+c.Text()); err != nil {
		bot.log.Error("support: deliver reply failed", "err", err)
		return c.Send("⚠️ Не удалось отправить ответ.")
	}
	if sessionKey == activeAdminReplySessionID(bot.adminID) {
...
func (bot *Bot) cancelAdminReply(ctx context.Context, c tele.Context) error {
	...
	bot.clearActiveReplyMessageID()
	if session != nil {
		if _, err := bot.b.EditReplyMarkup(c.Message(), markup.Reply(session.TargetTgID)); err != nil {
...
func (bot *Bot) findAdminReplySession(ctx context.Context, c tele.Context) (*admin.Session, int, error) {
