...
type UserUseCase struct {
	...
	hiddify subscription.Repository
	log     *slog.Logger
}

func NewUserUseCase(users user.Repository, hiddify subscription.Repository, log *slog.Logger) *UserUseCase {
	return &UserUseCase{
		users:   users,
		hiddify: hiddify,
...
func (uc *UserUseCase) upsertFromPanel(ctx context.Context, pu subscription.PanelUser, now time.Time, result *SyncResult) error {
...
func (uc *UserUseCase) ListUnboundPanelUsers(ctx context.Context) ([]subscription.PanelUser, error) {
	panelUsers, err := uc.hiddify.ListPanelUsers(ctx)
	if err != nil {
		return nil, fmt.Errorf("list unbound: %w", err)
	}

	out := make([]subscription.PanelUser, 0)
	for _, pu := range panelUsers {
		if pu.TelegramID == nil || *pu.TelegramID == 0 {
			out = append(out, pu)
		}
...
func (uc *UserUseCase) ApproveAccessRequest(ctx context.Context, telegramID int64, username string) (*subscription.CreatedUser, error) {
	name := username
	if name == "" {
		name = fmt.Sprintf("tg_%d", telegramID)
	}

	created, err := uc.hiddify.CreateUser(ctx, subscription.CreateUserRequest{
		Name:       name,
		TelegramID: telegramID,
	})
	if err != nil {
